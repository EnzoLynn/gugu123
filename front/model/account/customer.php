...
class ModelAccountCustomer extends Model {
	public function addCustomer($data) {
        ...
        $customer = array(
            ...
            'fullname'      => '',//前台注册，第一次姓名不填写
            'email'         => $data['email'],
            'telephone'    => $data['telephone'],
            'fax'          =>  '',
            'custom_field'=> isset($data['custom_field']) ? serialize($data['custom_field']) : '',
        ...
        $customer['password'] = user_password($customer);
	...
	public function editPassword($email, $password) {
        ...
        $query = $this->db_ci->from('customer')->where('email', $email)->get();
...
//	}
    ...
    public function addLoginAttempt($login_name) {
        $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "customer_login WHERE login_name = '" . $this->db->escape($login_name) . "' AND ip = '" . $this->db->escape($this->request->server['REMOTE_ADDR']) . "'");

        if (!$query->num_rows) {
            $this->db->query("INSERT INTO " . DB_PREFIX . "customer_login SET login_name = '" . $this->db->escape($login_name) . "', ip = '" . $this->db->escape($this->request->server['REMOTE_ADDR']) . "', total = 1, date_added = '" . $this->db->escape(date('Y-m-d H:i:s')) . "', date_modified = '" . $this->db->escape(date('Y-m-d H:i:s')) . "'");
        } else {
            $this->db->query("UPDATE " . DB_PREFIX . "customer_login SET total = (total + 1), date_modified = '" . $this->db->escape(date('Y-m-d H:i:s')) . "' WHERE customer_login_id = '" . (int)$query->row['customer_login_id'] . "'");
        }
    }

    public function getLoginAttempts($login_name) {
        $query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "customer_login` WHERE login_name = '" . $this->db->escape($login_name) . "'");

        return $query->row;
    }

    public function deleteLoginAttempts($login_name) {
        $this->db->query("DELETE FROM `" . DB_PREFIX . "customer_login` WHERE login_name = '" . $this->db->escape($login_name) . "'");
    }
}