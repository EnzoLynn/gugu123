<div class="menuHeader">
  <div class="navbar-header">
    <button type="button" class="catergroybtn navbar-toggle collapsed
                 visible-sm-block visible-xs-block" style="border:solid 1px #ccc">
      <span class="sr-only">分类导航栏</span>
      <span style="background-color: #888888;" class="icon-bar"></span>
      <span style="background-color: #888888;" class="icon-bar"></span>
      <span style="background-color: #888888;" class="icon-bar"></span>
    </button>
  </div>
</div>
<div class="row logFloor">
  <div class="col-sm-120 col-xs-120 col-md-20">
    <a href="#"><img class="img-responsive" src="/picBase/log.png" alt="" /></a>
  </div>
  <div class="col-md-100  hidden-sm hidden-xs">
    <div class="row ">
      <div class="col-md-20"></div>
      <div class="cartGroup col-md-70">
        <div class="left col-md-70">
          <input class="serInput" type="text" name="serInput" placeholder="Marshall暑期特惠" value="">
        </div>
        <div class="col-md-50">
          <input class="serBtn" type="image" src="/picBase/pics/search btn.png" onmousemove="this.src='/picBase/pics/search btn.png'" onmouseout="this.src='/picBase/pics/search btn.png'" />
        </div>
      </div>
      <div class="col-md-30">
        <div class="joinCartGrop text-right">
          <div class="joinc">
            <img class="cartImg" src='/picBase/pics/cart.png'>
            <span class="cartBtn">购物车</span>
            <div class="cart-f-div panel panel-default">
              <div class="panel-heading text-left">
                <span class="pro-num">0</span> 件商品</div>
              <div class="panel-body cart-pro-item-ct">
                <table class="pro-table text-left">
                  <tbody>
                  <!-- 参看 cart-pro-item -->
                  </tbody>
                </table>
              </div>
              <div class="panel-footer">
                <div class="cart-pro-total-ct text-right left">
                  <label>合计:</label><span class="cart-pro-total">0</span>
                  <label>元</label>
                </div>
                <button class="btn gotopay disabled" type="button">去结算</button>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
    <div class="cate-ul-container">
      <ul class="logmenu  hidden-sm hidden-xs row">
        <li class="col-md-15"><a href="javascript:void(0);" tab="#tab1">吉他</a></li>
        <li class="col-md-15"><a href="javascript:void(0);" tab="#tab2">贝司</a></li>
        <li class="col-md-15"><a href="javascript:void(0);" tab="#tab3">鼓</a></li>
        <li class="col-md-15"><a href="javascript:void(0);" tab="#tab4">音箱</a></li>
        <li class="col-md-20" style="min-width: 50px;"><a href="javascript:void(0);" tab="#tab5">效果器</a></li>
        <li class="col-md-20" style="min-width:70px;"><a href="javascript:void(0);" tab="#tab6">通用配件</a></li>
        <li class="col-md-20" style="min-width:70px;"><a href="javascript:void(0);" tab="#tab7">改装配件</a></li>
      </ul>
    </div>
  </div>
</div>

