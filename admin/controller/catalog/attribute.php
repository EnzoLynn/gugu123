...
class ControllerCatalogAttribute extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
            ...
            /**
             * 增加用户体验，连续添加某一个属性组的属性的时候，记录上一次的属性组选择，初期添加量大使用，后期属性和属性组基本添加完成了，可以去掉
             */
            setcookie('attribute_group_id', $this->request->post['attribute_group_id'], time() + 60 * 60 * 24 * 1);
	...
	protected function getList() {
        ...
        if (isset($this->request->get['attribute_group_id'])) {
            $data['filter_attribute_group_id'] = (int)$this->request->get['attribute_group_id'];
        }
        $filter_total_data = array();
        if (isset($this->request->get['attribute_group_id'])) {
            $filter_data['filter_attribute_group_id'] = (int)$this->request->get['attribute_group_id'];
            $filter_total_data['attribute_group_id'] = (int)$this->request->get['attribute_group_id'];
        }
		$attribute_total = $this->model_catalog_attribute->getTotalAttributes($filter_total_data);
	...
	protected function getForm() {
        ...
        if (isset($this->request->request['attribute_group_id'])) {
            $data['attribute_group_id'] = $this->request->request['attribute_group_id'];
	...
	protected function validateDelete() {
		...
		foreach ($this->request->post['selected'] as $attribute_id) {
			...
			if ($product_total) {
                ...
                $attribute = $this->model_catalog_attribute->getAttributeDescriptions($attribute_id);
                $attribute_name = $attribute[$this->config->get('config_language_id')]['name'];

                $product_ids = $this->model_catalog_product->getProductsByAttributeId($attribute_id);
                $product_names = array();
                foreach($product_ids as $product_id) {
                    $product = $this->model_catalog_product->getProduct($product_id);
                    $product_names[] = '<a href="'. $this->url->link('catalog/product/edit', 'token=' . $this->session->data['token'] . '&product_id=' . $product_id, 'SSL') .'">' . $product['name'] . '</a>';
                }
                $this->error['warning'] = sprintf($this->language->get('error_product'), $attribute_name, $product_total, implode(',', $product_names));
	...
	public function autocomplete() {
		...
		if (isset($this->request->get['filter_name']) && !empty($this->request->get['filter_name'])) {
			...
			//$results = $this->model_catalog_attribute->getAttributes($filter_data);
            $results = $this->model_catalog_attribute->getAttributesByGroupName($filter_data);
